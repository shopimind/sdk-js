/**
 * Contract snapshot: the endpoints the SDK emits and the `client_api` OpenAPI
 * routes that back them.
 *
 * The SDK's ONLY dependency is `client_api`'s OpenAPI (`/api-json`). This module
 * is the single source of truth for the contract, consumed by:
 *   - `openapi-coverage.test.ts` — proves SDK surface ⊆ OPENAPI_PATHS (or allow-list);
 *   - `check-openapi.ts` (`yarn check:openapi`) — diffs OPENAPI_PATHS against a
 *     freshly dumped `openapi.json` (route removed → CI failure; new route → info).
 *
 ...
 */
export const SDK_ENDPOINTS: readonly string[] = [
  // customers
...
/**
 * Parse an OpenAPI document's `paths` into the same `"VERB /path"` shape as
 * OPENAPI_PATHS. Shared by the diff script (H2). Accepts the parsed JSON object.
 */
export function extractOpenapiPaths(doc: unknown): Set<string> {
  ...
  for (const [route, item] of Object.entries(paths)) {
    ...
    for (const method of Object.keys(item)) {
      ...
      // Only real HTTP verbs (skip `parameters`, `$ref`, `summary`, …).
      if (
